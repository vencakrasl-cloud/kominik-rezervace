...
const GOOGLE_SHEETS_URL = 'https://script.google.com/macros/s/AKfycbwehJ0AZlb1NjC9Wesb221eAAsmxapkDqLuHlzq7mbVgtSNMrBQ4fd83z1utPeJP-rd/exec';
...
const SCOPES = 'https://www.googleapis.com/auth/calendar.readonly'; // Změna: jen čtení
...
const KominikReservation = () => {
  ...
  // Načtení existujících rezervací při startu
  useEffect(() => {
    // Načti rezervace z Google Calendar přes Web App
    loadCalendarEvents();
  }, []);
  
  // Načtení rezervací z Google Calendar přes Web App (JSONP)
  const loadCalendarEvents = async () => {
    try {
      console.log('🔄 Načítám události z kalendáře...');
      console.log('📍 URL:', GOOGLE_SHEETS_URL + '?action=getEvents');
      ...
      console.log('📄 Response text (prvních 500 znaků):', text.substring(0, 500));
      
      const data = JSON.parse(text);
      console.log('✅ Parsed data:', data);
      
      if (!data || !data.success) {
        console.error('❌ Chyba při načítání událostí:', data?.error);
        loadMockBookings();
        return;
      }
      
      console.log('📅 NAČTENO UDÁLOSTÍ:', data.events.length);
      
      // Převeď události z kalendáře na formát aplikace
      ...
      data.events.forEach(event => {
        ...
        if (isAllDay) {
          // CELODENNÍ UDÁLOST - blokuj celý den
          ...
          while (currentDate < endDate) {
            ...
            bookingsFromCalendar.push({
              date: dateStr,
              time: '08:00',
              address: event.title || 'Blokováno',
              lat: 50.7821,
              lon: 14.2148,
              duration: 540, // 9 hodin
              customerName: event.title || 'Blokováno - celý den',
          ...
          // Kontrola vícedenních událostí
          const startDate = start.toISOString().split('T')[0];
          const endDate = end.toISOString().split('T')[0];
          
          if (startDate !== endDate) {
            // VÍCEDENNÍ ČASOVÁ UDÁLOST
            ...
            while (currentDate <= finalDate) {
              ...
              bookingsFromCalendar.push({
                ...
                customerName: event.title || 'Vícedenní událost',
                googleEventId: event.id,
            ...
            // JEDNODENNÍ ČASOVÁ UDÁLOST
            bookingsFromCalendar.push({
              date: startDate,
              time: start.toTimeString().slice(0, 5),
              address: event.location || event.title || 'Rezervace',
              lat: lat,
              lon: lon,
              duration: duration,
              customerName: event.title || 'Rezervace',
              googleEventId: event.id
            });
          }
        }
      ...
      console.log('✅ CELKEM REZERVACÍ:', bookingsFromCalendar.length);
  ...
  const searchAddresses = async (query) => {
    try {
      ...
      const results = data.map(item => {
        // Rozdělení display_name a úprava názvů
        const parts = item.display_name
          ...
          .filter(p => 
            ...
            !p.includes('Northeast') &&
            !p.includes('Severozápad') &&
            !p.includes('Severovýchod') &&
            !p.includes('okres') &&
            p.length > 0
          )
          // Nahrazení názvů krajů
          .map(p => {
            if (p === 'Usti nad Labem Region' || p === 'Ústí nad Labem Region') {
              return 'Ústecký kraj';
            }
            if (p === 'Liberec Region' || p === 'Liberecký Region') {
        ...
        // Vytvoř zkrácenou verzi bez Czechia/Northwest/Northeast
  ...
  const handleSubmit = async () => {
    ...
    const newBooking = {
      ...
      chimneysCount: formData.chimneysCount,
      fireplaceCleaning: formData.fireplaceCleaning,
      notes: formData.notes || ''
    };
    
    try {
      // Odeslání do Google Sheets přes Web App
      const response = await fetch(GOOGLE_SHEETS_URL, {
        method: 'POST',
        mode: 'no-cors', // Důležité pro Google Apps Script
        headers: {
      ...
      // Poznámka: no-cors vrací opaque response, takže nemůžeme číst odpověď
      // Předpokládáme, že to proběhlo úspěšně
      
      // Přidej do lokálního stavu (pro okamžité blokování)
      const updatedBookings = [...bookings, {
        ...newBooking,
        address: formData.city // Pro kompatibilitu se stávající logikou
      }];
      ...
      alert('Nepodařilo se odeslat rezervaci. Zkuste to prosím znovu.');
      setSubmitting(false);
    }
  };
