...
const GOOGLE_SHEETS_URL = 'https://script.google.com/macros/s/https://script.google.com/macros/s/AKfycbw9pshqgVLAIqOJvKWCqKAdR6V4qEJnlUjEmZQbTD-556GqC8n4O76liVCF5T8WnAca/exec/exec'; // Nahraďte svým deployment URL
...
const KominikReservation = () => {
  ...
  // Inicializace Google Identity Services (nová metoda)
  const initGoogleClient = () => {
    // Inicializace Google API Client (pro Calendar API)
    window.gapi.client.init({
      apiKey: '', // Není potřeba pro OAuth
  ...
  // Google Sign In s novou GIS metodou
    ...
    window.gapi.client.calendar.events.list({
      ...
      events.forEach(event => {
        ...
        if (isAllDay) {
          ...
          // Projdi všechny dny v rozmezí
          const currentDate = new Date(startDate);
          while (currentDate < endDate) {
            const dateStr = currentDate.toISOString().split('T')[0];
            
            // Vytvoř "blokující" událost pro celý pracovní den (8:00-17:00)
            bookingsFromCalendar.push({
              ...
              lat: 50.7821,
              lon: 14.2148,
              duration: 540, // 9 hodin (8:00-17:00)
              customerName: event.summary || 'Blokováno - celý den',
